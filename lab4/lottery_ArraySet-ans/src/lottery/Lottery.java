...
public class Lottery {
	...
	public void reset()
	{
		...
		tickets = new ArraySet<>();
		// reset the existing winning numbers
		winningNumbers = new ArraySet<>(maxPick);
	...
	public int sellTicket()
	{
		 ...
		 */
		LotteryTicket ticket = new LotteryTicket(++numOfTicketsSold, this);
		tickets.add(ticket);
		return ticket.id();
	}
	...
	public int sellTicket(SetADT<Integer> numbers)
	{
		 ...
		 * 		the set of tickets sold so far.
		 */
		LotteryTicket ticket = new LotteryTicket(++numOfTicketsSold, this, numbers);
		tickets.add(ticket);
		return ticket.id();
	}
	...
	public int[] drawNumbers()
	{
		...
		{
			int pick = availableNumbers.removeRandom();
			winningNumbers.add(pick);
			results[i] = pick;
		}
	...
	public String hasWon()
	{
		...
		{
			SetADT<Integer> purchasedNumbers = ticket.chosenNumbers();
			SetADT<Integer> results = purchasedNumbers.intersection(winningNumbers);
			if(results.size() == maxPick)
			{
