...
public class MotherLink {
	...
	public void addNextInLine(Female daughter)
	{
		 ...
		 */
		LinearNode<Female> node = new LinearNode<>(daughter);
	...
	public String toString()
	{
		 ...
		 */
		/* As size() is an O(n) operation, we only do it once and
		 * keep its result in a local variable for later use.
		 */
		int size = size(); 		
		switch (size) {
		/* As there is only one constructor in this class which always 
		 * creates a LinearNode with an element in it, there is no chance
		 * case 0 would occur, but we include it here for completeness and
		 * for future-proof (i.e. in case a new constructor is to be added
		 * to the code in future).  
		 */
		case 0 : return results + " is empty.";  
		case 1 : results += "has one person. \n She is: \n"; break;
		default : results += "has " + size + " people. \n They are: \n"; break;
		}
		...
		while (current != null)
		{
			/* Output a number of tabs depends how far up the ancestry line 
			 * 	the current female is.
			 */ 
